...
ReScript를 CommonJS 또는 ES6 모듈로 컴파일하는지에 따라 **이 기능은 미묘하게 다른 코드를 생성합니다.** 차이점을 보려면 두 출력 탭을 모두 확인하세요. 여기의 ES6 출력은 틀리게 나올겁니다!
...
ReScript의 관용적 [가져오기 및 내보내기](https://green-labs.github.io/rescript-in-korean/Language-Features/22-Import-Export)에서 언급했듯이, 모든 let 바인딩 및 모듈은 기본적으로 다른 ReScript 모듈로 내보내집니다(`.resi` [인터페이스 파일](https://green-labs.github.io/rescript-in-korean/Language-Features/21-Module)을 사용하지 않는 한). 컴파일된 JS 파일을 열면, 이러한 값을 _JavaScript_ 파일에서도 직접 사용할 수 있음을 알 수 있습니다.

### ES6 기본값(Default) 내보내기
...
만약 자바스크립트의 ES6 기본 가져오기가 Babel/Webpack/Jest에 의해 CommonJS `require`로 변환되는 경우도 저희는 처리했습니다. `__esModule`에 대한 CommonJS 출력 탭을 참조하세요.
